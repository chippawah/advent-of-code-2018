...
Keep map of coordinates
map = { '<X>,<Y>': <CLAIM COUNT> }

Keep overall duplicated claims
Increment only when a map[coordinate] === 2

Fill map using starting coordinates and area
*/
  ...
  for (let i = 0; i < claims_arr.length; i++) {
    ...
    for (let x = x_start ; x < x_start + width ; x++) {
      for (let y = y_start ; y < y_start + height ; y++) {
        const map_key = `${x},${y}`
        if (!map[map_key]) { map[map_key] = 0 }
        map[map_key]++
        if (map[map_key] === 2) { duplicated++ }
      }
    }
  }
  return duplicated
