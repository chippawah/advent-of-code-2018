...
function first_dup_freq(arr = [], start_freq = 0, start_idx = 0, initial_map = {}) {
  const map = initial_map
  let frequency = start_freq
  for (let i = start_idx; i < arr.length; i++) {
    frequency += arr[i]
    if (!map[frequency]){ map[frequency] = 0 }
    map[frequency]++
    if (map[frequency] === 2) { return frequency }
  }
  return first_dup_freq([...arr, ...arr], frequency, arr.length, map)
}

export default first_dup_freq
